...
import { RefreshCw, AlertCircle, Share2, BarChart2 } from 'lucide-react'
...
const CLIENT = CLIENTS.excelencia
...
export default function ExcelenciaDashboard() {
  ...
  const fetchMeta = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/meta?since=${since}&until=${until}&creatives=true&accountId=${CLIENT.metaAccountId}`
      )
  ...
  const fetchGoogle = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/google?since=${since}&until=${until}&customerId=${CLIENT.googleCustomerId}`
      )
      if (!res.ok) {
        const data = await res.json()
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-zinc-950">
      <header className="bg-white dark:bg-zinc-900 border-b border-gray-200 dark:border-zinc-800 sticky top-0 z-10">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-2 flex items-center gap-1.5 overflow-x-auto scrollbar-none">
          ...
          {getRecentMonths(6).reverse().filter(({ year }) => year >= 2026).map(({ year, month }) => (
            <button
              key={`${year}-${month}`}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        {tab === 'meta' && metaData && (
          <div className="space-y-10">
            ...
            {metaData.campaigns.filter(c => c.spend > 0).length > 0 && (
              <div>
                <h2 className="text-sm font-bold text-gray-800 dark:text-zinc-100 pb-2.5 border-b border-gray-200 dark:border-zinc-700 mb-4">
                  Campanhas ({metaData.campaigns.filter(c => c.spend > 0).length})
                </h2>
                <MetaCampaigns
                  campaigns={metaData.campaigns.filter(c => c.spend > 0)}
                  creatives={creatives}
                />
              </div>
            )}
